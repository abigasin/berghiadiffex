...
class blast():
	...
	def find_sequences(self,fastapath):
		print('You have '+str(len(self.genes[0]))+' genes of interest.')
	...
	def execute_blast(self,records,output,newlist):
   		...
   		for i in np.arange(len(self.newlist)):
   			print("Blasting gene " + self.newlist[i]+ " against the NR database.")
   			result_handle = NCBIWWW.qblast('blastx','nr',self.records[self.newlist[i]].seq,format_type='Text',hitlist_size=15,expect=0.0001,entrez_query='metazoa[Organism]')
...
if __name__=="__main__":
	...
	dat.find_sequences(dat.fastapath)
